...
5. **Documentation Updates** — Updated architecture.md, api.md, CLAUDE.md with security hardening details (JWT algorithm whitelist, role gate, password rate limiting, OAuth timeout, CSRF constant-time).

6. **Stale File Cleanup** — Removed stale docker-compose.override.yml, config.sh.example, duplicate SQL, dead Dockerfile build args, deprecated env var fallbacks, dead exports. Updated README and troubleshooting docs.
...
**Deprecated:** `NEXT_PUBLIC_ADMIN_PASSWORD`, `NEXT_PUBLIC_API_KEY` (removed in Session 6)
...
## Stale File Cleanup (Session 6)

Scanned the entire repo for stale, redundant, or deprecated files. Fixed 8 issues:

### Deleted Files
- `docker-compose.override.yml` — Stale override that silently disabled built-in bot, dropped web healthchecks, and missed new auth env vars
- `config.sh.example` — Referenced bitable env vars (`REMINDER_APP_TOKEN`, `REMINDER_TABLE_ID`) no longer used since PostgreSQL migration

### Fixed Issues
1. **Duplicate `user_sessions` table in `db/init.sql`** — Identical CREATE TABLE + indexes appeared twice (lines 67-76 and 80-90); removed duplicate
2. **Dead Dockerfile build args** (`packages/web/Dockerfile`) — Removed `NEXT_PUBLIC_API_KEY` and `NEXT_PUBLIC_ADMIN_PASSWORD` ARG/ENV lines (auth moved to server-side JWT)
3. **Deprecated env var fallback** (`packages/server/src/routes/auth.js`) — Removed `|| process.env.NEXT_PUBLIC_ADMIN_PASSWORD` fallback; only `ADMIN_PASSWORD` now
4. **Dead `apiAuth` export** (`packages/server/src/middleware/auth.js`) — Removed legacy alias `apiAuth: sessionAuth` (no production code imported it)
5. **Test alias cleanup** (`packages/server/tests/auth.test.js`) — Replaced all `apiAuth` references with `sessionAuth` directly
6. **Stale README** (`README.md`) — Updated env vars section (replaced NEXT_PUBLIC vars with JWT_SECRET/ADMIN_PASSWORD), updated troubleshooting table
7. **Stale troubleshooting** (`docs/troubleshooting.md`) — Replaced two sections about NEXT_PUBLIC build-time injection with JWT-based troubleshooting
